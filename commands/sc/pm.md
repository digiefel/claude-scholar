...
- **State Questions**: "How far did we get?", "Current status", "Progress update" trigger context report
- **Vague Requests**: "I want to build", "I want to implement", "How should I" trigger discovery mode
...
4. Act (Improvement):
   - Success → docs/patterns/[pattern-name].md (formal write-up)
...
Error Detection Protocol:
  ...
  2. Root Cause Investigation (MANDATORY):
     ...
     → Document: "The cause of the error appears to be [X], because [evidence Y]"

  3. Hypothesis Formation:
     - Create docs/pdca/[feature]/hypothesis-error-fix.md
     - State: "Cause: [X]. Evidence: [Y]. Solution: [Z]"
     - Rationale: "[why this approach will resolve the issue]"
...
Anti-Patterns (Strictly Forbidden):
  ...
  ❌ "There's a warning but it works so it's fine" (future technical debt)

Correct Patterns (Required):
  ✅ "An error occurred. Research the official documentation"
  ✅ "Cause: environment variable not set. Why is it needed? Understand the spec"
  ✅ "Solution: add .env + implement startup validation"
  ✅ "Learning: check environment variables first from now on"
```
...
execution/:
  execution/[feature]/do        # Do phase: experiments and trial-and-error
...
Structure (clear and easy to understand):
  docs/pdca/[feature-name]/
    ...
    ├── do.md             # Do: experiments and trial-and-error
...
After each successful implementation:
  - Create docs/patterns/[feature-name].md (formal write-up)
